...
void ft_send_msg(int sockfd, int fd, char *msg)
{
	...
	for (int i = 0; i <= max_fd; i++)
	{
		if (FD_ISSET(i, &fds_active) && i != sockfd && i != fd)
		{
...
{
	char *to_print;
	int ret_extract_fct = extract_message(&outbuf[fd], &to_print);
	while (ret_extract_fct)
	{
		ft_send_msg(sockfd, fd, to_print);
		free(to_print);
		ret_extract_fct = extract_message(&outbuf[fd], &to_print);
	}
	if (ret_extract_fct == -1)
	{
		DEBUG_PRINT("Error: extract_message failed\n");
		ft_err_exit(sockfd);
	}
}
